...
DbManager::DbManager() {
	...
	db_manager.setDatabaseName("european_cities.db");
...
void DbManager::initDataBase() {
	...
	if (!itemsFile.open(QIODevice::ReadOnly)) qDebug() << itemsFile.errorString();

	// the header of the distance file is read, so these values are not stored in
	// the db
	distancesFile.readLine();

	// the header of the iteems file is read, so these values are not stored in
	// the db
	itemsFile.readLine();
	...
	QVector<QString>
			cities;    // holds the names of the cities with in the distance file
	QString city;  // used for storing the name of the city that which the items
								 // and costs belong too
	...
	while (!itemsFile.atEnd()) {
		// from the QByteArray variable, it splits it based on commas and only
		// inserts the first element into the string (city)
		city = itemsLine.split(',').at(0);

		// reads the next that holds the items and the cost value
		itemsLine = itemsFile.readLine();

		// stores the items and cost, by spiiting itemsLine by commas and accessing
		...
		while (tempItem != NULL && !itemsFile.atEnd()) {
			...
			tempItem =
					itemsLine.split(',').at(1);  // stores the item into the tempVariable
			tempCost =
					itemsLine.split(',').at(2);  // stores the item into the tempVariable
		}
		// since the while loop exits out when the file is at the last line, we add
		// the push back the last elements that were on that line
		if (itemsFile.atEnd()) {
			items.push_back(tempItem);  // stores the final item
			costs.push_back(tempCost);  // stores the final item
		}

		// for loop - since the city is know the for loop is used to traverse
		// throught the vector and dump the values inside the database
		for (int index = 0; index < items.size(); index++) {
			query.prepare(
					"INSERT INTO Items(City, Item, Cost) VALUES (:val1, :val2, :val3);");
	...
	while (!distancesFile.atEnd()) {
		// from the QByteArray varibable, it is split based on commas and then
		// assigned to each string based on the index within the array
		...
		query.prepare(
				"INSERT INTO Distances(Starting, Ending, Distance) VALUES "
				"(:val1, :val2, :val3);");
		...
		/*
		 * The following lines of code are for creating a list of the cities that
		 * are within the file. Checking first if the vector to hold the cities is
		 * empty. Then running a foor loop & iterating through the vector to check
		 * if the starting cities name is already inside the vector. If found, the
		 * boolean variable 'check' is assigned to true. But if its not found then
		 * in the last if statement it will push back the city into the cities
		 * vector. Finally the boolean variable 'check' is re-assigned to false for
		 * the next run.
		 */

		// if the vector for cities is empty (meaning no city is in it) then push
		// the city into the citises vector
		if (cities.empty()) cities.push_back(startingCity);

		// const run time - checks the whole vector if an element(city) exists that
		// with the same name
		for (QVector<QString>::iterator it = cities.begin(); it != cities.end();
				 it++)
			if (*it == startingCity)
		...
		if (!check) cities.push_back(startingCity);

		check = false;
	}

	// since the while loop exits out when the file is at the last line, theres
	// still an element on the last line that doesnt get inserted to the database
